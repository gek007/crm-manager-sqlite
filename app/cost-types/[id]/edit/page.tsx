...
export default function EditCostTypePage() {
  ...
  if (loading) {
    return <AppLayout><Header title="Edit Cost Type" /><div className="p-6"><p className="text-muted-foreground text-center py-8">Loading...</p></div></AppLayout>;
  }
  if (!item) {
    return <AppLayout><Header title="Edit Cost Type" /><div className="p-6"><p className="text-destructive text-center py-8">Cost type not found.</p></div></AppLayout>;
  }

  return (
    <AppLayout>
      <Header title="Edit Cost Type" />
      <div className="p-6">
        <Card className="max-w-2xl border-border/50">
          <CardHeader><CardTitle className="text-lg">Edit Cost Type</CardTitle></CardHeader>
          <CardContent>
            <form action={formAction} className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <SubmitButton>Update Cost Type</SubmitButton>
                <a href="/cost-types"><Button type="button" variant="secondary">Cancel</Button></a>
              </div>
            </form>
          </CardContent>
        </Card>
